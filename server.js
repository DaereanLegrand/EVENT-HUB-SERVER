...
function editarEvento(response, nombre, lugar, categoria, startdate, enddate,
  ...
  const query = `
    UPDATE eventos
    SET lugar='${lugar}', categoria='${categoria}', fcomienzo='${
      startdate}', ffin='${enddate}', hcomienzo='${starttime}', hfin='${
      endtime}', descripcion='${descripcion}'
    WHERE nombre='${nombre}'
  `;
  console.log(query);
...
<<<<<<< HEAD
  ...
  case "/crearEvento":
    ...
    request.on("end", function() {
      let params = JSON.parse(body);
      console.log(params);
      crearEvento(response, params.nombre, params.lugar, params.startdate,
                  params.enddate, params.starttime, params.endtime,
                  params.descripcion);
    })
    break;
  case "/crearAmbiente":
